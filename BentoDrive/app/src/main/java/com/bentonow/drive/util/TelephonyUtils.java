...
public class TelephonyUtils {
    ...
    public static CallStatusModel getInformation(Context context) {
        CallStatusModel mCallStatus = new CallStatusModel();
        network = getNetworkInfo(context);

        if (network == null || !network.isConnectedOrConnecting()) {
            // Sys.log("kokusho", "No connection");
            mCallStatus.setConexion(false);
            mCallStatus.setTipoConexion("N/A");
        } else {
            String sConnection = typeConection(network.getType(), network.getSubtype());
            mCallStatus.setTipoConexion(sConnection);
        }

        mCallStatus.setImei(getImei(context));
        ...
        try {
            mCallStatus.setOperator(getCarrierName(context));
        } catch (Exception ex) {
        ...
        try {
            mCallStatus.setMcc(networkOperator.substring(0, 3));
        } catch (Exception ex) {
            DebugUtils.logError(TAG, ex);
        }

        try {
            mCallStatus.setMcn(networkOperator.substring(3));
        } catch (Exception ex) {
            DebugUtils.logError(TAG, ex);
        }

        try {
            mCallStatus.setCellLocation(cellLocation.toString());
        } catch (Exception ex) {
            DebugUtils.logError(TAG, ex);
        }
        ...
        try {
            ...
            mCallStatus.setCell(String.valueOf(cid));
        } catch (Exception ex) {
            DebugUtils.logError(TAG, ex);
        }
        ...
        try {
            ...
            mCallStatus.setGsmLocation(String.valueOf(lac));
        } catch (Exception ex) {
            DebugUtils.logError(TAG, ex);
        }

        try {
            mCallStatus.setCountryCode(telephonyManager.getNetworkCountryIso());
        } catch (Exception ex) {
            DebugUtils.logError(TAG, ex);
        }

        try {
            mCallStatus.setHour(getHora());
        } catch (Exception ex) {
            DebugUtils.logError(TAG, ex);
        }

        try {
            mCallStatus.setMinute(getMinute());
        } catch (Exception ex) {
            DebugUtils.logError(TAG, ex);
        }

        try {
            mCallStatus.setSecond(getSecond());
        } catch (Exception ex) {
            DebugUtils.logError(TAG, ex);
        }

        try {
            mCallStatus.setModel(android.os.Build.MODEL);
        } catch (Exception ex) {
            DebugUtils.logError(TAG, ex);
        }

        try {
            mCallStatus.setVersionAndroid(android.os.Build.VERSION.RELEASE);
        } catch (Exception ex) {
            DebugUtils.logError(TAG, ex);
        }
        ...
        try {
            ...
            mCallStatus.setLat(lat);
            mCallStatus.setLon(lon);
        } catch (Exception ex) {
            DebugUtils.logError(TAG, ex);
        }

        try {
            mCallStatus.setStrenght(strenght);
        } catch (Exception ex) {
            DebugUtils.logError(TAG, ex);
        }
